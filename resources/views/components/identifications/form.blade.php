...
<div class="row">
    <div class="col-12 form-group">
    	...
    	<div class="row">
    		@forelse( $types->where('model', $model ?? '')->get() AS $type )
				<div class="col-md-{{ $loop->count > 1 && $loop->count < 5 ? ( 12 / $loop->count ) : 6 }}">
				    <x-form.input 
				    	type="text" 
				    	name="external[{{ $type->id }}]"
				    	class="form-control mb-2 {{ $type->input_classes }}" 
				    	attr="{!! $type->input_params !!}" 
				    	data-field="externals[{{ $type->id }}].value" 
				    	label="{{ $type->name }}"
				    />
				</div>
